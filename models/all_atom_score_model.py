...
from models.tensor_layers import get_irrep_seq
...
class TensorProductScoreModel(torch.nn.Module):
    ...
    def forward(self, data):
        if self.crop_beyond is not None:
            ...
            data['receptor'].pos = complex_graph['receptor'].pos[residues_to_keep]
            data['receptor'].x = complex_graph['receptor'].x[residues_to_keep]
            data['receptor'].side_chain_vecs = complex_graph['receptor'].side_chain_vecs[residues_to_keep]
            data['receptor', 'rec_contact', 'receptor'].edge_index = subgraph(residues_to_keep, complex_graph['receptor', 'rec_contact', 'receptor'].edge_index, relabel_nodes=True)[0]
