...
class InferenceDataset(Dataset):
    ...
    def get(self, idx):
        ...
        try:
            ...
            rec, rec_coords, c_alpha_coords, n_coords, c_coords, misc_coords_array, misc_features_array, lm_embeddings, sequences \
                = extract_receptor_structure(rec_model, mol, cutoff=100000, lm_embedding_chains=lm_embedding)
            if lm_embeddings is not None and len(c_alpha_coords) != len(lm_embeddings):
                print(f'LM embeddings for complex {name} did not have the right length for the protein. Skipping {name}.')
                complex_graph['success'] = False
                return complex_graph

            get_rec_graph(rec, rec_coords, c_alpha_coords, n_coords, c_coords, misc_coords=misc_coords_array, misc_features=misc_features_array, complex_graph=complex_graph, rec_radius=self.receptor_radius,
...
class ScreeningDataset(Dataset):
    def __init__(self, protein_file, ligand_smiles, lm_embeddings,
                 receptor_radius=30, c_alpha_max_neighbors=None, precomputed_lm_embeddings=None,
                 remove_hs=False, all_atoms=False, atom_radius=5, atom_max_neighbors=None):

        super(ScreeningDataset, self).__init__()
        self.receptor_radius = receptor_radius
        self.c_alpha_max_neighbors = c_alpha_max_neighbors
        self.remove_hs = remove_hs
        self.all_atoms = all_atoms
        ...
        # generate LM embeddings
        if lm_embeddings and precomputed_lm_embeddings is None:
            print("Generating ESM language model embeddings")
            model_location = "esm2_t33_650M_UR50D"
            model, alphabet = pretrained.load_model_and_alphabet(model_location)
            model.eval()
            ...
            protein_sequence = get_sequences_from_pdbfile(protein_file)
            sequences = protein_sequence.split(':')
            labels = ['chain_' + str(j) for j in range(len(sequences))]
            lm_embeddings = compute_ESM_embeddings(model, alphabet, labels, sequences)
            lm_embeddings = [lm_embeddings[f'chain_{j}'] for j in range(len(sequences))]

            with open('data/lm_embeddings.pkl', 'wb') as f:
                pickle.dump(lm_embeddings, f)

        elif lm_embeddings and precomputed_lm_embeddings is not None:
            with open(precomputed_lm_embeddings, 'rb') as f:
        ...
        rec_model = parse_pdb_from_path(protein_file)
        rec, rec_coords, c_alpha_coords, n_coords, c_coords, misc_coords_array, misc_features_array, lm_embeddings, sequences \
            = extract_receptor_structure(rec_model, None, cutoff=100000, lm_embedding_chains=lm_embeddings)
        if lm_embeddings is not None and len(c_alpha_coords) != len(lm_embeddings):
            raise Exception(f'LM embeddings did not have the right length for the protein.')
        get_rec_graph(rec, rec_coords, c_alpha_coords, n_coords, c_coords, misc_coords=misc_coords_array, misc_features=misc_features_array, complex_graph=complex_graph, rec_radius=self.receptor_radius,
                      c_alpha_max_neighbors=self.c_alpha_max_neighbors, all_atoms=self.all_atoms,
                      atom_radius=self.atom_radius, atom_max_neighbors=self.atom_max_neighbors, remove_hs=self.remove_hs, lm_embeddings=lm_embeddings)
    ...
    def len(self):
        return len(self.ligand_smiles)

    def get(self, idx):
        ligand_graph = HeteroData()
        smile = self.ligand_smiles[idx]
        try:
            mol = MolFromSmiles(smile)
            mol = AddHs(mol)
            generate_conformer(mol)
        except Exception as e:
            print('Failed to generate initial structure of molecule ', smile, ' We are skipping it. The reason is the exception: ', e)
            ligand_graph.success = False
            return ligand_graph

        get_lig_graph_with_matching(mol, ligand_graph, popsize=None, maxiter=None, matching=False, keep_original=False,
                                    num_conformers=1, remove_hs=self.remove_hs)

        complex_graph = copy.deepcopy(self.complex_graph)
